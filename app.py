...
if "lat" not in st.session_state: st.session_state.lat = 28.613504
if "lon" not in st.session_state: st.session_state.lon = 77.216541
...
with st.sidebar:
    ...
    opencage_key = st.text_input("OpenCage API Key:", type="password")
...
if clicked_lat and clicked_lon:
    st.markdown(
        f"<p style='text-align:center;'>Lat: {clicked_lat:.6f}, Lon: {clicked_lon:.6f}</p>",
        unsafe_allow_html=True
    )
else:
    st.markdown(
        "<p style='text-align:center;'>Click anywhere on the map to get coordinates.</p>",
        unsafe_allow_html=True
    )
...
with chat_col:
    st.header("💬 Chat About This Location")
